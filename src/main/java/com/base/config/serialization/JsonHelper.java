...
public class JsonHelper {
	...
	public void unsupportedParameters(final Type typeOfMap, final String json,
		...
		if (!unsupportedParameterList.isEmpty()) {
			String message = messageSource.getMessage("validation.unsupported.parameters",
					...
					"Unsupported parameters: " + String.join(", ", unsupportedParameterList), locale);
	...
	public String extractString(final String fieldName, final JsonElement element) {
		...
		if (!valueElement.isJsonPrimitive() || !valueElement.getAsJsonPrimitive().isString()) {
			throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error", "Value of param must be string",
					fieldName);
		}
	...
	public Set<Long> extractArrayAsLong(final String fieldName, final JsonElement element) {
		...
		if (!valueElement.isJsonArray()) {
			throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
					"Value of param must be an array of numbers", fieldName);
		}
		...
		for (JsonElement item : array) {
			if (!item.isJsonPrimitive() || !item.getAsJsonPrimitive().isNumber()) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
						"All elements of array must be numbers", fieldName);
			}
			try {
				result.add(item.getAsLong());
			}
			catch (NumberFormatException ex) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error", "Invalid number format in array",
						fieldName);
			}
		}
	...
	public Long extractLong(final String fieldName, final JsonElement element) {
		...
		if (!valueElement.isJsonPrimitive() || !valueElement.getAsJsonPrimitive().isNumber()) {
			throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error", "Value of param must be number",
					fieldName);
		}
	...
	public boolean parameterExists(final String fieldName, final JsonElement element) {
		return element != null && element.getAsJsonObject().has(fieldName)
				&& !element.getAsJsonObject().get(fieldName).isJsonNull();
	}
	...
	public List<JsonObject> extractArrayAsObject(final String fieldName, final JsonElement element) {
		...
		if (!valueElement.isJsonArray()) {
			throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
					"Value of param must be an array of objects", fieldName);
		}
		...
		for (JsonElement item : array) {
			if (!item.isJsonObject()) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
						"All elements of array must be objects", fieldName);
			}
