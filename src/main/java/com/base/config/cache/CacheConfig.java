...
public class CacheConfig {
  ...
  private static final String[] CACHE_NAMES =
      new String[] {"users", "roles", "offices", "provinces", "districts", "communes", "villages"};
  ...
  private CacheManager createRedisCacheManager(RedisConnectionFactory connectionFactory) {
    try {
      ...
      return new CustomRedisCacheManager(
          RedisCacheWriter.nonLockingRedisCacheWriter(connectionFactory), cacheDefaults);

    } catch (Exception e) {
      logger.warn(
          "Failed to create redis cache manager, falling back to caffeine cache manager", e);
      return createCaffeineCacheManager();
    }
  }
