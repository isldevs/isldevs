...
public class JwtBearerAuthenticationProvider implements AuthenticationProvider {
	...
	public JwtBearerAuthenticationProvider(JwtDecoder jwtDecoder, RegisteredClientRepository registeredClientRepository,
	...
	public Authentication authenticate(Authentication authentication) {
		...
		if (!jwt.getSubject().contains(registeredClient.getClientId())) {
			throw new OAuth2AuthenticationException(
					new OAuth2Error(OAuth2ErrorCodes.INVALID_REQUEST, "Invalid client", null));
		}
		...
		var authorizationBuilder = OAuth2Authorization.withRegisteredClient(registeredClient)
			...
			.token(new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, accessTokenValue, Instant.now(),
		...
		var accessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, oAuth2Token.getTokenValue(),
				oAuth2Token.getIssuedAt(), oAuth2Token.getExpiresAt());
		...
		return new OAuth2AccessTokenAuthenticationToken(registeredClient, jwtBearerToken, accessToken, null,
