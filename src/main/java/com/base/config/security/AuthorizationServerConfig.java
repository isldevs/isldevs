...
public class AuthorizationServerConfig {
	...
	public AuthorizationServerConfig(final MessageSource messageSource,
			final AuthenticationProvider authenticationProvider, final OAuth2AuthorizationService authorizationService,
			final OAuth2TokenGenerator<?> tokenGenerator, final RegisteredClientRepository registeredClientRepository,
			...
			final CorsConfigurationSource corsConfigurationSource, final UserInfoService userInfoService,
		...
		http.with(configurer, (authorizationServer) -> authorizationServer
			...
			.tokenEndpoint(tokenEndpoint -> tokenEndpoint
				.accessTokenRequestConverter(new DelegatingAuthenticationConverter(
						Arrays.asList(new CustomAuthenticationConverter(), new OAuth2PasswordAuthenticationConverter(),
				...
				.authenticationProviders(providers -> {
					...
					providers.add(new OAuth2PasswordAuthenticationProvider(authenticationProvider, authorizationService,
							tokenGenerator, registeredClientRepository));
					providers.add(new OAuth2RefreshTokenAuthenticationProvider(authorizationService, tokenGenerator));
					providers
						.add(new OAuth2ClientCredentialsAuthenticationProvider(authorizationService, tokenGenerator));
				})));

		http.securityMatcher(configurer.getEndpointsMatcher())
			.cors(cors -> cors.configurationSource(corsConfigurationSource))
			.csrf(csrf -> csrf.ignoringRequestMatchers("/api/v1/oauth2/token", "/api/v1/oauth2/device_authorization",
					"/api/v1/oauth2/token/introspect", "/api/v1/oauth2/token/revoke"))
			.authorizeHttpRequests(authorize -> authorize
				.requestMatchers("/api/v1/oauth2/token", "/api/v1/oauth2/device_authorization",
						"/api/v1/oauth2/token/introspect", "/api/v1/oauth2/token/revoke",
						"/.well-known/oauth-authorization-server", "/.well-known/openid-configuration", "/jwks",
						"/api/v1/device/**", "/css/**", "/js/**", "/api/v1/login/**", "/api/v1/error/**",
						"/api/v1/public/**")
				.permitAll()
				.anyRequest()
				.authenticated())
			...
			.headers(headers -> headers.xssProtection(Customizer.withDefaults())
				...
				.contentSecurityPolicy(csp -> csp.policyDirectives("default-src 'self'; " + "script-src 'self'; "
						...
						+ "form-action 'self'; " + "frame-ancestors 'none'; " + "block-all-mixed-content"))
				.frameOptions(HeadersConfigurer.FrameOptionsConfig::deny)
				.contentTypeOptions(Customizer.withDefaults())
				.httpStrictTransportSecurity(hsts -> hsts.includeSubDomains(true).maxAgeInSeconds(31536000))
				.permissionsPolicyHeader(
						permissions -> permissions.policy("geolocation 'none'; midi 'none'; camera 'none'"))
				.referrerPolicy(referrerPolicyConfig -> referrerPolicyConfig
					.policy(ReferrerPolicyHeaderWriter.ReferrerPolicy.STRICT_ORIGIN_WHEN_CROSS_ORIGIN)))
			.redirectToHttps(
					redirectToHttp -> redirectToHttp.requestMatchers(r -> r.getHeader("X-Forwarded-Proto") != null));
