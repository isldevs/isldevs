...
public class ResourceServerConfig {
	...
	@Autowired
	public ResourceServerConfig(final JwtDecoder jwtDecoder, final SessionRegistry sessionRegistry,
			final CustomAuthenticationEntryPoint authenticationEntryPoint,
			final ClientRegistrationRepository clientRegistrationRepository,
			final OAuth2AuthorizedClientService oauth2AuthorizedClientService,
			final OAuth2UserServiceImpl oauth2UserService, final OidcUserServiceImpl oidcUserService,
	...
	public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(auth -> auth
			.requestMatchers("/api/v1/oauth2/**", "/api/v1/device/**", "/css/**", "/js/**", "/api/v1/login/**",
					"/api/v1/error/**", "/api/v1/public/**")
			.permitAll()
			.anyRequest()
			.authenticated())
			...
			.oauth2Login(oauth2 -> oauth2.clientRegistrationRepository(clientRegistrationRepository)
				...
				.tokenEndpoint(
						tokenEndpointConfig -> tokenEndpointConfig.accessTokenResponseClient(accessTokenResponseClient))
				.userInfoEndpoint(userInfo -> userInfo.userService(oauth2UserService).oidcUserService(oidcUserService))
				.successHandler(authenticationSuccessHandler)
				.loginPage("/login")
				.permitAll()
				.defaultSuccessUrl("/home", true)
