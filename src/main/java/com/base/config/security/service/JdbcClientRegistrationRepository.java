...
public class JdbcClientRegistrationRepository implements ClientRegistrationRepository {
  ...
  public ClientRegistration findByRegistrationId(String registrationId) {
    List<ClientRegistration> list =
        jdbcTemplate.query(
            ...
            (rs, rowMapper) ->
                ClientRegistration.withRegistrationId(rs.getString("registration_id"))
                    .clientId(rs.getString("client_id"))
                    .clientSecret(rs.getString("client_secret"))
                    .clientAuthenticationMethod(
                        ClientAuthenticationMethod.valueOf(rs.getString("client_auth_method")))
                    .authorizationGrantType(
                        new AuthorizationGrantType(rs.getString("authorization_grant_type")))
                    .redirectUri(rs.getString("redirect_uri"))
                    .scope(
                        rs.getString("scope") != null
                            ? rs.getString("scope").split(",")
                            : new String[] {})
  ...
  public void save(ClientRegistration client) {
    jdbcTemplate.update(
        ...
        client.getClientAuthenticationMethod().getValue(),
        client.getAuthorizationGrantType().getValue(),
        client.getRedirectUri(),
        String.join(",", client.getScopes()),
        client.getProviderDetails().getAuthorizationUri(),
