...
public class JdbcClientRegistrationRepository implements ClientRegistrationRepository {
	...
	public ClientRegistration findByRegistrationId(String registrationId) {
		List<ClientRegistration> list = jdbcTemplate.query(
				...
				(rs, rowMapper) -> ClientRegistration.withRegistrationId(rs.getString("registration_id"))
					...
					.scope(rs.getString("scope") != null ? rs.getString("scope").split(",") : new String[] {})
	...
	public void save(ClientRegistration client) {
		jdbcTemplate.update("INSERT INTO oauth2_client_registration "
				...
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", client.getRegistrationId(), client.getClientId(),
				client.getClientSecret(), client.getClientAuthenticationMethod().getValue(),
				client.getAuthorizationGrantType().getValue(), client.getRedirectUri(),
				String.join(",", client.getScopes()), client.getProviderDetails().getAuthorizationUri(),
				client.getProviderDetails().getTokenUri(), client.getProviderDetails().getUserInfoEndpoint().getUri(),
				client.getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName(), client.getClientName(),
