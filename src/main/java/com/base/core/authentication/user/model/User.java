...
public class User extends CustomAbstractPersistable implements UserDetails, Serializable {
	...
	public static User from(JsonCommand command, String username, PasswordEncoder passwordEncoder,
	...
	public static class Builder {
		...
		public Builder authority(Collection<? extends GrantedAuthority> authorities) {
			this.roles = authorities.stream().map(grantedAuthority -> {
				Role role = Role.builder().name(grantedAuthority.getAuthority()).build();
				Set<Authority> authoritySet = new HashSet<>();
				Authority authority = Authority.builder().authority(grantedAuthority.getAuthority()).build();
	...
	public Map<String, Object> changed(PasswordEncoder passwordEncoder, RoleRepository roleRepository,
			JsonCommand command) {
		...
		if (command.isChangeAsArray(UserConstants.ROLES,
				this.roles.stream().map(Role::getName).collect(Collectors.toSet()), String.class)) {
