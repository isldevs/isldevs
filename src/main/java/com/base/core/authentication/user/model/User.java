...
public class User extends CustomAbstractPersistable implements UserDetails, Serializable {
          ...
          authorities.stream()
              .map(
                  grantedAuthority -> {
                    ...
                    Authority authority =
                        Authority.builder().authority(grantedAuthority.getAuthority()).build();
  ...
  public Map<String, Object> changed(
      PasswordEncoder passwordEncoder, RoleRepository roleRepository, JsonCommand command) {
    ...
    if (command.isChangeAsArray(
        UserConstants.ROLES,
        this.roles.stream().map(Role::getName).collect(Collectors.toSet()),
        String.class)) {
