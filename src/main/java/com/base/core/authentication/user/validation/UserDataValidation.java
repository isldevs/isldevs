...
public class UserDataValidation {
    ...
    public void create(String json) {
        ...
        if (this.jsonHelper.parameterExists(UserConstants.ENABLED, jsonElement)) {
            ...
            validator.parameter(UserConstants.ENABLED, enabled)
                    .isBoolean();
        }
        if (this.jsonHelper.parameterExists(UserConstants.IS_ACCOUNT_NON_EXPIRED, jsonElement)) {
            final var isAccountNonExpired = this.jsonHelper.extractBoolean(UserConstants.IS_ACCOUNT_NON_EXPIRED, jsonElement);
            validator.parameter(UserConstants.IS_ACCOUNT_NON_EXPIRED, isAccountNonExpired)
                    .isBoolean();
        }
        if (this.jsonHelper.parameterExists(UserConstants.IS_ACCOUNT_NON_LOCKED, jsonElement)) {
            final var isAccountNonLocked = this.jsonHelper.extractBoolean(UserConstants.IS_ACCOUNT_NON_LOCKED, jsonElement);
            validator.parameter(UserConstants.IS_ACCOUNT_NON_LOCKED, isAccountNonLocked)
                    .isBoolean();
        }
        if (this.jsonHelper.parameterExists(UserConstants.IS_CREDENTIALS_NON_EXPIRED, jsonElement)) {
            final var isCredentialsNonExpired = this.jsonHelper.extractBoolean(UserConstants.IS_CREDENTIALS_NON_EXPIRED, jsonElement);
            validator.parameter(UserConstants.IS_CREDENTIALS_NON_EXPIRED, isCredentialsNonExpired)
                    .isBoolean();
        }
    }
