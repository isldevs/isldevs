...
public class RoleCommandHandler extends CommandBuilder {
	...
	public CommandBuilder update(final Long id) {
		return this.action("UPDATE").entity(RoleConstants.PERMISSION).id(id).href(RoleConstants.API_PATH + "/" + id);
	}

	public CommandBuilder delete(final Long id) {
		return this.action("DELETE").entity(RoleConstants.PERMISSION).id(id).href(RoleConstants.API_PATH + "/" + id);
	}
