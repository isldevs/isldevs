...
public class GlobalExceptionHandler {
  ...
  public GlobalExceptionHandler(MessageSource messageSource, SecurityContext securityContext) {
  ...
  public ResponseEntity<ErrorData> handleMethodNotSupported(
      HttpRequestMethodNotSupportedException ex, Locale locale) {
    var localizedMessage = messageSource.getMessage(ex.getMessage(), null, ex.getMessage(), locale);
    return buildResponseEntity(
        HttpStatus.METHOD_NOT_ALLOWED, localizedMessage, ex.getMessage(), null);
  }

  @ExceptionHandler(HttpMediaTypeNotSupportedException.class)
  public ResponseEntity<ErrorData> handleMediaTypeNotSupported(
      HttpMediaTypeNotSupportedException ex, Locale locale) {
    var localizedMessage = messageSource.getMessage(ex.getMessage(), null, ex.getMessage(), locale);
    return buildResponseEntity(
        HttpStatus.UNSUPPORTED_MEDIA_TYPE, localizedMessage, ex.getMessage(), null);
  }
  ...
  public ResponseEntity<ErrorData> handleGenericException(Exception ex, Locale locale) {
    var localizedMessage = messageSource.getMessage(ex.getMessage(), null, ex.getMessage(), locale);
    return buildResponseEntity(
        HttpStatus.INTERNAL_SERVER_ERROR, localizedMessage, ex.getMessage(), null);
  }
  ...
  public ResponseEntity<ErrorData> invalidDataAccessResourceUsageException(
      InvalidDataAccessResourceUsageException ex, Locale locale) {
    var localizedMessage =
        messageSource.getMessage("msg.internal.error", null, ex.getMessage(), locale);
    return buildResponseEntity(
        HttpStatus.INTERNAL_SERVER_ERROR, localizedMessage, ex.getMessage(), null);
  }

  @ExceptionHandler(value = {AuthorizationDeniedException.class})
  public ResponseEntity<ErrorData> handleAccessDeniedException(
      AuthorizationDeniedException ex, Locale locale) {
    List<String> args = extract(ex);
    String localizedMessage =
        messageSource.getMessage(
            "msg.access.denied", args != null ? args.toArray() : null, ex.getMessage(), locale);
    return buildResponseEntity(
  ...
  public ResponseEntity<ErrorData> handleDataIntegrityViolationException(
  ...
  public ResponseEntity<ErrorData> handleDataIntegrityViolationException(
      UnexpectedRollbackException ex, Locale locale) {
    String localizedMessage = messageSource.getMessage("msg.transaction.rollback", null, locale);
    return buildResponseEntity(HttpStatus.BAD_REQUEST, localizedMessage, ex.getMessage(), null);
  }

  @ExceptionHandler(AuthenticationException.class)
  public ResponseEntity<ErrorData> handleAuthenticationException(
  ...
  public ResponseEntity<ErrorData> handleNotFoundException(NotFoundException ex, Locale locale) {
    var localizedMessage =
        messageSource.getMessage(ex.getMessage(), ex.getArgs(), ex.getMessage(), locale);
    return buildResponseEntity(
        HttpStatus.NOT_FOUND, localizedMessage, ex.getMessage(), ex.getArgs());
  }
  ...
  public ResponseEntity<ErrorData> handleNotFoundException(
    ...
    var localizedMessage =
        messageSource.getMessage("validation.file.size", maxSizeMB, ex.getMessage(), locale);
    return buildResponseEntity(
        HttpStatus.PAYLOAD_TOO_LARGE, localizedMessage, ex.getMessage(), maxSizeMB);
  }

  @ExceptionHandler(NullPointerException.class)
  public ResponseEntity<ErrorData> handleNotFoundException(NullPointerException ex, Locale locale) {
    var localizedMessage =
        messageSource.getMessage("msg.internal.error", ex.getSuppressed(), locale);
    return buildResponseEntity(
        HttpStatus.INTERNAL_SERVER_ERROR, localizedMessage, ex.getMessage(), ex.getSuppressed());
  }
  ...
  private Object[] extractDuplicateArgs(Object[] args) {
    ...
    return Arrays.stream(args).filter(Objects::nonNull).toList().toArray();
  }
  ...
  private ResponseEntity<ErrorData> buildResponseEntity(
      HttpStatus status, String message, String error, Object[] args) {
