	...
	public static class Validator {
		...
		public Validator isBoolean() {
			if (value != null && !(value instanceof Boolean)) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error", "Value of param must be boolean",
						param);
			}
			return this;
		}
		...
		public Validator notEmpty() {
			if (value.toString().trim().isEmpty()) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
						"Value of param must not be empty", param);
			}
			return this;
		}
		...
		public Validator notNull() {
			if (value == null) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
						"Value of param must not be null", param);
			}
			return this;
		}
		...
		public Validator maxLength(int max) {
			if (value != null && value.toString().length() > max) {
				throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
						"Value of param must not be exceed " + max + " characters", param);
			}
			return this;
		}
				...
				if (value instanceof Collection<?>) {
					for (Object item : (Collection<?>) value) {
						if (!(item instanceof Number)) {
							throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
									"All elements of param must be numbers", param);
						}
					}
				}
					...
					throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
							"Value of param must be an array of numbers", param);
				}
			}
			return this;
		}
			...
			if (value != null) {
				if (value instanceof Number num) {
					if (num.doubleValue() <= 0) {
						throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
								"Value of param must be positive", param);
					}
				}
				else {
					throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
		...
		public Validator between(Number min, Number max) {
			if (value instanceof Number num) {
				...
				if (v < min.doubleValue() || v > max.doubleValue()) {
					throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error",
							"Value of param must be between " + min + " and " + max, param);
				}
			}
			return this;
		}
