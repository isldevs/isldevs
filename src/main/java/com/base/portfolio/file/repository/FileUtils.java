...
public class FileUtils {
  ...
  public static void isValidEntityName(final String entity) {
    ...
    throw new ErrorException(
        HttpStatus.BAD_REQUEST, "msg.internal.error", "Entity type is unsupported", entity);
  }
  ...
  static InputStream resize(String extension, InputStream is) {
    try (ByteArrayOutputStream bos = new ByteArrayOutputStream();
      ...
      if (src == null) {
        throw new ErrorException(
            HttpStatus.INTERNAL_SERVER_ERROR, "msg.internal.error", "Can not read file content");
      }
  ...
  public static void isValidateMimeType(final String mimeType) {
    ...
    boolean isValid =
        Arrays.stream(MIME_TYPE.values())
            .anyMatch(type -> type.getValue().equalsIgnoreCase(mimeType));

    if (!isValid) {
      throw new ErrorException(
          HttpStatus.BAD_REQUEST, "msg.internal.error", "Unsupported MIME type", mimeType);
    }
  }
  ...
  public enum FILE_EXTENSION {
    ...
    public FILE_EXTENSION extension() {
      return switch (this) {
        ...
        default ->
            throw new ErrorException(
                HttpStatus.UNSUPPORTED_MEDIA_TYPE, "msg.internal.error", "Unsupported Media Type");
      };
    }
  }
  ...
  public static byte[] byteArray(File file) throws IOException {
    if (file == null || !file.exists() || !file.isFile()) {
      throw new ErrorException(
          HttpStatus.INTERNAL_SERVER_ERROR, "msg.internal.error", "Invalid file provided");
    }
    ...
    try (FileInputStream fis = new FileInputStream(file)) {
      ...
      if (bytesRead != fileBytes.length) {
        throw new ErrorException(
            HttpStatus.INTERNAL_SERVER_ERROR, "msg.internal.error", "File read length mismatch");
      }
    }
