...
public class FileUtils {
	...
	static InputStream resize(String extension, InputStream is) {
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream(); InputStream input = is) {
			...
			throw new ErrorException(HttpStatus.INTERNAL_SERVER_ERROR, "msg.internal.error", "Resizing failed",
					e.getMessage());
		}
	}
	...
	public static void isValidateMimeType(final String mimeType) {
		if (mimeType == null || mimeType.isBlank()) {
			throw new ErrorException(HttpStatus.BAD_REQUEST, "msg.internal.error", "MIME type cannot be null or empty",
					mimeType);
		}
	...
	public enum MIME_TYPE {
		...
		JPEG("image/jpeg"), JPG("image/jpg"), PNG("image/png"), PDF("application/pdf"),
		...
		public static MIME_TYPE extension(FILE_EXTENSION fileExtension) {
			return switch (fileExtension) {
				...
				default -> throw new ErrorException(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "msg.internal.error",
	...
	public enum URI_SUFFIX {
		...
		JPG("data:" + MIME_TYPE.JPG.getValue() + ";base64,"), JPEG("data:" + MIME_TYPE.JPEG.getValue() + ";base64,"),
		PNG("data:" + MIME_TYPE.PNG.getValue() + ";base64,"), PDF("data:" + MIME_TYPE.PDF + ";base64,"),
		XLSX("data:" + MIME_TYPE.XLSX + ";base64,"), DOCX("data:" + MIME_TYPE.DOCX + ";base64,");
