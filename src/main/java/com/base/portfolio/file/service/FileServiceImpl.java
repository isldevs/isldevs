...
import com.base.portfolio.file.repository.FileRepository;
import com.base.portfolio.file.repository.FileUtils;
import com.base.portfolio.file.repository.Storage;
import com.base.portfolio.file.validation.FileDataValidation;
...
public class FileServiceImpl implements FileService {
	...
	@Autowired
	public FileServiceImpl(final Storage storage, final FileDataValidation validator, final MessageSource messageSource,
	...
	public Map<String, Object> uploadFile(JsonCommand command) {
		...
		try {
			...
			if (entityFile == null) {
				entityFile = File.builder().entity(command.getEntityType()).entityId(command.getEntityId()).build();
			}
			else {
				oldFileName = entityFile.getName();
			}

			var directory = storage.writeFile(file.getInputStream(), command.getEntityId(), command.getEntityType(),
					file.getOriginalFilename(), oldFileName, storage);
			...
			return LogData.builder().id(entityFile.getId()).success("msg.success", messageSource).build().claims();

		}
		catch (IOException e) {
			throw new ErrorException(HttpStatus.INTERNAL_SERVER_ERROR, "msg.internal.error",
	...
	public Map<String, Object> delete(final JsonCommand command) {
		...
		var dir = Paths.get(FileConstants.DIR, command.getEntityType(), String.valueOf(command.getEntityId()),
				file.getName());
		storage.deleteFile(dir);
		this.repository.delete(file);
		this.repository.flush();
		return LogData.builder().success("msg.success", messageSource).build().claims();
	}
	...
	public Map<String, Object> fileURL(String entity, Long entityId) {
		...
		return LogData.builder().file(url).success("msg.success", messageSource).build().claims();
	}
	...
	public Map<String, Object> fileBase64(String entity, Long entityId) {
		...
		return LogData.builder().file(base64).success("msg.success", messageSource).build().claims();
	}
	...
	public Map<String, Object> fileByte(String entity, Long entityId) {
		...
		return LogData.builder().file(bytes).success("msg.success", messageSource).build().claims();
	}
	...
	public Map<String, Object> fileInputStream(String entity, Long entityId) {
		...
		return LogData.builder().file(inputStream).success("msg.success", messageSource).build().claims();
	}
